# Spring Security — Complete Revision Notes (Merged: Concepts + Full Project Code)
...
```
**Fix:** Always use `PreparedStatement` / parameterized queries — never string-concatenate user input into SQL. (Spring Data JPA, used later in this project, parameterizes queries for you automatically.)
...
- **Default password:** randomly generated on every application start, printed in the console:
  ...
  ```
  > ⚠️ This message explicitly says: *"This generated password is for development use only. Your security configuration must be updated before running the application production."*
...
- This property-based approach is only read while you're using Spring's **default** `UserDetailsService`/`SecurityFilterChain`. The moment you define your own beans (Section 12 onward), this property is ignored.
...
- You can also choose **JWT** as an auth type in Postman — not covered in this module but mentioned as a future/stateless alternative.
...
- Test flow from the course: wrong password → rejected; each hardcoded user's own username/password → works. Still not from a database — that's next.

---

## 11. Database-Backed Authentication — Setup
...
# MySQL equivalent (change port/driver accordingly, default MySQL port is 3306)
...
# PART B — Full Buildable Project (File Structure + Complete Code)

Everything below is the **final, complete state** of the demo project after all the incremental steps above have been applied — laid out as an actual project you can copy into your IDE and run.
...
**Package-by-layer rationale:**
- `config` → Spring Security configuration (`SecurityFilterChain`, `AuthenticationProvider` beans)
- `controller` → REST endpoints
- `dao` → JPA repository interfaces (database access)
- `model` → entities + Spring Security wrapper classes (`UserPrincipal`)
- `service` → business logic layer (`UserDetailsService` impl, registration logic)

---

## 18. `pom.xml` — Key Dependencies
...
<dependencies>
    ...
    <!-- JPA (for DB-backed users) -->
    <dependency>
        <groupId>org.springframework.boot</groupId>
        <artifactId>spring-boot-starter-data-jpa</artifactId>
    </dependency>

    <!-- PostgreSQL driver (swap for mysql-connector-j if using MySQL) -->
    <dependency>
        <groupId>org.postgresql</groupId>
        <artifactId>postgresql</artifactId>
        <scope>runtime</scope>
...
```
> After editing `pom.xml`, always **reload Maven** in your IDE, then **restart** the app (don't rely purely on DevTools hot-reload when dependencies change).

---

## 19. `application.properties` — Complete
...
# --- MySQL equivalent (comment out Postgres block above, use this instead) ---
# spring.datasource.url=jdbc:mysql://localhost:3306/telusko
...
# --- CSRF cookie hardening (extra layer, optional alongside CSRF token) ---
server.servlet.session.cookie.same-site=strict
```
> Note: `spring.security.user.name` / `spring.security.user.password` properties (Section 6) are used **only** in the early "hardcoded single user" stage of the course. Once you define your own `UserDetailsService` / `SecurityFilterChain` bean, these properties are ignored.

---

## 20. Database Setup (SQL) — Complete

```sql
-- Create the users table
CREATE TABLE users (
    id       INTEGER PRIMARY KEY,
    username TEXT UNIQUE,   -- recommended: unique, since login looks up by username
    password TEXT
);

-- Sample seed data (passwords initially plain text; BCrypt-encoded after Section 14)
INSERT INTO users (id, username, password) VALUES (1, 'navin', 'n@123');
INSERT INTO users (id, username, password) VALUES (2, 'kiran', 'n@789');
INSERT INTO users (id, username, password) VALUES (3, 'harsh', 'h@123');
...
## 21. `HelloController.java` — Complete
...
public class HelloController {
    ...
    @GetMapping("/hello")
    public String greet(HttpServletRequest request) {
        // request.getSession().getId() is only for learning/demo — never log session IDs in production
        return "Hello World " + request.getSession().getId();
    }
...
## 22. `Student.java` (model) — Complete
...
## 23. `StudentController.java` — Complete (demonstrates CSRF behavior)
...
public class StudentController {
    ...
    // Fetch a fresh CSRF token to use in the X-CSRF-TOKEN header for POST/PUT/DELETE requests
    @GetMapping("/csrf-token")
    public CsrfToken getCsrfToken(HttpServletRequest request) {
        return (CsrfToken) request.getAttribute("_csrf");
    }
...
## 24. `User.java` (JPA Entity) — Complete
...
@Entity
@Table(name = "users")   // table is "users" (plural) but class is "User" (singular) — must map explicitly
@Data
@AllArgsConstructor
@NoArgsConstructor
public class User {
...
## 25. `UserRepo.java` (JPA Repository) — Complete
...
## 26. `UserPrincipal.java` (implements `UserDetails`) — Complete
...
public class UserPrincipal implements UserDetails {
    ...
    @Override
    public Collection<? extends GrantedAuthority> getAuthorities() {
        // Hardcoded role for demo. In a real app, add a "role"/"authority" column
        // to the users table and return it dynamically here instead.
        return Collections.singleton(new SimpleGrantedAuthority("USER"));
    }
    ...
    public boolean isAccountNonExpired() {
        return true; // not implemented in this demo — always true
    }
...
## 27. `MyUserDetailsService.java` (implements `UserDetailsService`) — Complete
...
## 28. `UserService.java` (Registration + BCrypt Encoding) — Complete
...
## 29. `UserController.java` (Registration Endpoint) — Complete
...
## 30. `SecurityConfig.java` — Complete Final Version
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            // ^ uncomment for a fully stateless REST API (no session cookie, CSRF not needed at all)
    ...
    public AuthenticationProvider authProvider() {
        DaoAuthenticationProvider provider = new DaoAuthenticationProvider();
        provider.setUserDetailsService(userDetailsService);
        provider.setPasswordEncoder(new BCryptPasswordEncoder(12)); // must match the strength used at registration
        return provider;
    }
}
```
> ⚠️ **Critical rule:** the BCrypt strength (`12` above) used by the `AuthenticationProvider` for **verifying** logins must be consistent with what's used in `UserService` for **encoding** at registration. Mismatched strengths won't break verification (BCrypt reads the round count embedded in the stored hash itself), but keep them equal across the app for clarity and consistency.

---

## 31. `SpringSecDemoApplication.java` — Complete
...
## 32. Full Authentication Flow (End-to-End Diagram)

```
REGISTRATION
------------
POST /register  (JSON body: {id, username, password})
   → UserController.register()
   → UserService.saveUser()
        - BCryptPasswordEncoder.encode(rawPassword)
   → UserRepo.save(user)
   → users table (password column now stores a BCrypt hash, e.g. $2a$12$...)


LOGIN / AUTHENTICATION
-----------------------
Client sends request with Basic Auth (username:password)
   → Spring Security FilterChain
   → DaoAuthenticationProvider.authenticate(authentication)
        → calls UserDetailsService.loadUserByUsername(username)
              → MyUserDetailsService
                    → UserRepo.findByUsername(username)
                    → if null: throw UsernameNotFoundException ("User 404")
                    → else: wrap User into new UserPrincipal(user)
        → provider compares raw password vs UserPrincipal.getPassword()
              using BCryptPasswordEncoder.matches(raw, storedHash)
   → success → request proceeds to Controller
   → failure → 401 Unauthorized ("Bad credentials")
```

---

## 33. Postman Testing Cheat-Sheet

| Action | Method | URL | Auth | Body / Headers |
|---|---|---|---|---|
| Register a user | POST | `/register` | Basic Auth (any valid existing user) | `{"id":3,"username":"harsh","password":"h@123"}` |
| Get all students | GET | `/students` | Basic Auth | — |
| Add a student | POST | `/students` | Basic Auth + `X-CSRF-TOKEN` header (if CSRF enabled) | `{"id":3,"name":"Harsh","technology":"JavaScript"}` |
| Fetch CSRF token | GET | `/csrf-token` | Basic Auth | copy token value from response |
| Test hello endpoint | GET | `/hello` | Basic Auth | — |

**401 Unauthorized causes to check:**
1. Wrong/missing username-password (Basic Auth tab in Postman).
2. Missing `X-CSRF-TOKEN` header on a non-GET request (only if CSRF is enabled).
3. Password stored in DB doesn't match encoding scheme currently configured (e.g., DB has plain text but `AuthenticationProvider` expects BCrypt).

---

## 34. Useful External Resources
